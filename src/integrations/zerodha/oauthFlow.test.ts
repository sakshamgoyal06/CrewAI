...
vi.mock("../../pillars/wealth/zerodha/kiteEnv.js", () => ({
  ...
  kiteLoginBaseUrl: () => "https://kite.zerodha.com",
}));
...
describe("kite oauth flow", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    appCreds.mockResolvedValue({ apiKey: "kite_key", apiSecret: "kite_secret" });
  });
  ...
  it("mints a login URL with api_key and redirect_params state", async () => {
    ...
    expect(appCreds).toHaveBeenCalledWith("user-1");
  });

  it("fails when user has no app credentials", async () => {
