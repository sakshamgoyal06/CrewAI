...
export type UserHealthProfileRow = {
  ...
  next_question: "fitness" | "diet" | "timing" | "restrictions" | "done";
...
We’ll do four short questions. Reply in your own words; you can say "skip" anytime to finish with what we have so far.
...
export async function fetchUserHealthProfile(
  ...
  const { data, error } = await supabase
    ...
    .select(
      "id, user_profile_id, onboarding_completed_at, next_question, fitness_goals, diet_preferences, meal_timing_notes, dietary_restrictions",
    )
...
export async function runHealthOnboardingTurn(
  ...
  switch (row.next_question) {
    ...
    case "restrictions": {
      ...
      const fin = await updateProfile(input.userProfileId, {
        dietary_restrictions: msg,
        onboarding_completed_at: new Date().toISOString(),
        next_question: "done",
      });
