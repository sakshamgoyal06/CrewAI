...
const COMMAND_MAP: Record<string, CommandDef> = {
  ...
  journal: { intent: "HEALTH", department: "nutrition" },
  hevy: { intent: "HEALTH", department: "workouts" },
...
export const DEFAULT_SLASH_PROMPTS: Partial<Record<string, string>> = {
  ...
  hevy: "hevy routine: … (create) | hevy routine update: <uuid> — … (edit) | hevy workout: … (log). Use exercise names from your Hevy library.",
  journal:
    "End-of-day health journal — how you feel, training, sleep, food (e.g. rest day, Push B skipped, muscle ache).",
...
export const TELEGRAM_BOT_COMMANDS: readonly TelegramBotCommand[] = [
  ...
  { command: "hevy", description: "Create Hevy routine or log workout (Hevy Pro API)" },
  { command: "journal", description: "End-of-day health journal (training, sleep, recovery)" },
