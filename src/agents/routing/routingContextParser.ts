...
export type RoutingContextSignals = {
  ...
  /** User reports having done a planned commitment ("I went to gym") — close the event-log row. */
  looks_like_activity_completion_report: boolean;
...
export const NEUTRAL_ROUTING_CONTEXT: RoutingContextSignals = {
  ...
  looks_like_activity_completion_report: false,
...
- **looks_like_activity_completion_report**: user states they **already did** a scheduled activity ("I went to gym", "made it to the pool", "did my walk"). Past tense and affirmative only — false for plans ("going to gym later"), questions, or misses ("skipped the gym"). Can be true alongside other signals; the commitment gets closed out regardless of which pillar answers.
...
function parseRoutingJson(text: string): RoutingContextSignals | null {
  ...
  try {
    ...
    return {
      ...
      looks_like_activity_completion_report: asBool(raw.looks_like_activity_completion_report),
