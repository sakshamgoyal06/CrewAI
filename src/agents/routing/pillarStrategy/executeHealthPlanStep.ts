...
export async function executeHealthPlanStep(
  ...
  switch (cap) {
    ...
    case "journal": {
      const r = await tryHealthJournalAgent(stepCtx);
      return (
        r ?? {
    ...
    case "alternates": {
      const r = await tryAlternatesRecommenderAgent(stepCtx);
      return r ?? runNutritionCapability(stepCtx);
    }
    ...
    case "energy": {
      const r = await tryEnergyAgent(stepCtx);
      return (
        r ?? {
          text: HEALTH_GENERIC_ACK,
          metadata: { specialist: "HealthGeneric", genericAck: true },
    ...
    case "long_term_planning": {
      const r = await tryLongTermHealthPlanningAgent(stepCtx);
      return (
        r ?? {
          text: HEALTH_GENERIC_ACK,
          metadata: { specialist: "HealthGeneric", genericAck: true },
