...
7. **Memory** — Loaded once per turn: recent chat, rolling summary, semantic facts, structured profile/goals/logs, **active projects block** in user knowledge. **Accountability Agent** at orchestrator exit: `action_ledger` + `accountability` metadata on tool turns. Tunable via `MAGNUS_MEMORY_*`.
8. **Persistence** — `magnus_chat_messages` gets a user row and an assistant row per turn, with
...
**Last updated:** 2026-09-06 (multi-user scoping, pillarPhilosophy, minimal mode core)
