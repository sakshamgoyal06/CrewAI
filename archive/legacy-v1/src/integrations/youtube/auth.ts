...
function platformOAuthReady(): boolean {
...
export async function youtubeOauthConfiguredForUser(
  ...
  return Boolean(integrations.googleYoutubeRefreshToken);
}
...
export async function getAuthenticatedYoutubeClient(userProfileId?: string): Promise<{
  ...
  const integrations = await loadUserIntegrations(userProfileId);
  const refreshToken = integrations.googleYoutubeRefreshToken;
  ...
  if (refreshToken) {
    ...
    throw new Error(
      "YouTube is not authenticated for this user. Run `npm run youtube:auth` locally, then upsert google_youtube_refresh_token via scripts/upsert-user-integrations.mts.",
    );
  } else {
