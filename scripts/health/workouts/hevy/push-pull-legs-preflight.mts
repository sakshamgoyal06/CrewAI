...
const pushBMap: Mapping[] = [
  pick("Bench Press", (t) => /bench press \(barbell\)/i.test(t.title), () => 10, "Standard barbell bench press."),
  pick("Chest Press Machine", (t) => /chest press \(machine\)/i.test(t.title), () => 10, "Chest Press (Machine) — same template as Push A."),
  pick("Cable Fly Crossovers", (t) => /cable fly crossover/i.test(t.title), () => 10, "Cable Fly Crossovers — same as Push A."),
  pick("Seated Overhead Press Dumbbell", (t) => /seated overhead press \(dumbbell\)/i.test(t.title), () => 10, "Seated Overhead Press (Dumbbell) — same as Push A."),
  pick("Dumbbell Lateral Raise", (t) => /lateral raise \(dumbbell\)/i.test(t.title), () => 10, "Lateral Raise (Dumbbell)."),
  pick("Assisted Triceps Dip", (t) => /triceps dip \(assisted\)/i.test(t.title), () => 10, "Triceps Dip (Assisted) — same as Push A."),
  pick(
    "Triceps Rope Pushdown",
    (t) => /triceps rope pushdown/i.test(t.title),
    () => 20,
    "Exact match: Triceps Rope Pushdown.",
  ),
  pick("Crunch", (t) => /^Crunch$/i.test(t.title), () => 10, "Crunch — same as Cardio day."),
